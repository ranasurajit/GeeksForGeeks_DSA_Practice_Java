...
class Solution {
    ...
    Node cloneGraph(Node node) {
        ...
        Map<Node, Node> map = new HashMap<Node, Node>(); // SC: O(V)
        map.put(node, cloned);
        dfsGraph(node, cloned, map);
        return cloned;
    }
     ...
     * SC: O(V)
     */
    private void dfsGraph(Node node, Node cloned, Map<Node, Node> map) {
        for (Node v : node.neighbors) {
            if (!map.containsKey(v)) {
                Node cloneV = new Node(v.val);
                map.put(v, cloneV);
                cloned.neighbors.add(cloneV);
                dfsGraph(v, cloneV, map);
            } else {
                cloned.neighbors.add(map.get(v));
            }
        }
    }
}
