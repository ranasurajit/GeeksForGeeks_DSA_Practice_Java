...
class Solution {
    ...
    public Node cloneLinkedList(Node head) {
        // storing the references of each nodes in new and old linked-list in Map
        Map<Node, Node> map = new HashMap<Node, Node>(); // SC: O(N)
        Node dummy = new Node(-1);
        Node current = head;
        Node temp = dummy;
        // copying the LinkedList without random pointers
        ...
        // adding reference to the random pointers
        current = head;
        temp = dummy.next;
        while (current != null) { // TC: O(N)
